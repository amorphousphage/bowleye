...
class CoordinatorWorker:
    ...
    def Handler_processing(self, frame):
        ...
        #self.tracking_camera_worker.join()
        self.pins_camera_worker.stop()
        #self.pins_camera_worker.join()
        ...
        future_export = self.executor.submit(
            ...
            os.path.join('videos', f'pins_new_{self.lane_number}.mp4')
        )
...
class ScoreReader:
    def run(self, ref_frame, reading_frame, pin_coordinates, settings, parent=None):
        ...
        standing_pins = scorer.PinScoreReading(self.ref_frame, self.reading_frame, self.settings['pins_blurred_kernel'], self.settings['pins_blurred_sigma'], self.settings['pins_binary_threshold'], 255, self.settings['pin_reading_size'])

        # Print the result
        print(f"The Score is {len(standing_pins)}. The Pins left standing are: ", standing_pins)
